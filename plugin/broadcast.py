...
• `{i}add <id/reply to list/none>`
    Agregue el chat a la base de datos. Agrega el chat actual si no se especifica una identificación.

• `{i}rem <all/id/none>`
    Elimina el chat especificado (chat actual si no se especifica ninguno), o todos los chats.

• `{i}broadcast <reply to msg>`
    Envíe el mensaje respondido a todos los chats en la base de datos.

• `{i}forward <reply to msg>`
     Reenviar el mensaje a todos los chats en la base de datos.

• `{i}listchannels`
    Para obtener una lista de todos los chats agregados.
"""
...
async def broadcast_remover(event):
    ...
    if chat_id == "all":
        await x.edit("`Removiendo...`")
        udB.delete("BROADCAST")
        await x.edit("Database Limpiada.")
        return
    if is_channel_added(chat_id):
        rem_channel(chat_id)
        await x.edit("Eliminado de la base de datos")
        await asyncio.sleep(3)
        await x.delete()
        ...
        await x.edit("El canal ya está eliminado de la base de datos. ")
        await asyncio.sleep(3)
        await x.delete()
...
async def list_all(event):
    ...
    msg = "Canales en la base de datos:\n"
...
async def forw(event):
    if not event.is_reply:
        await eor(event, "Responder a un mensaje para transmitir.")
        return
    channels = get_channels()
    x = await eor(event, "Enviando...")
    if get_no_channels() == 0:
        return await x.edit(f"Agregue canales usando `{hndlr}add` En ellos.")
    error_count = 0
    sent_count = 0
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
    ...
    await x.edit(f"{sent_count} mensajes enviados con {error_count} errors.")
    if error_count > 0:
        try:
            await ultroid_bot.send_message(Var.LOG_CHANNEL, f"{error_count} Errors")
        except BaseException:
            await x.edit("Configure el canal de registro para verificar errores.")


@ultroid_cmd(pattern="broadcast ?(.*)", allow_sudo=False)
async def sending(event):
    x = await eor(event, "`Procesando...`")
    if not event.is_reply:
        return await x.edit("Responder a un mensaje para transmitir.")
    channels = get_channels()
    error_count = 0
    sent_count = 0
    if get_no_channels() == 0:
    ...
    if event.reply_to_msg_id:
        ...
        if (
            ...
            await x.edit(f"No soportado. Intentar `{hndlr}forward`")
            return
            ...
            for channel in channels:
                try:
                    ...
                    try:
                        ...
                        if (
                            error
                            == "El mensaje no puede estar vacío a menos que se proporcione un archivo"
                        ):
                            return await x.edit(
                                f"Para enviar archivos, cárguelos en Mensajes guardados y responda {hndlr}forward to in.",
                            )
                    except BaseException:
                        pass
                    error_count += 1

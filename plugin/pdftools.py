...
• `{i}pdf <page num> <reply to pdf file>`
    Extraer y enviar la página como una imagen. (Nota: para la extracción, todas las páginas solo usan .pdf)

• `{i}pdtext <page num> <reply to pdf file>`
    Extraer texto del PDF. (Nota: para extraer todo el texto, simplemente use .pdtext)

• `{i}pdscan <reply to image>`
    Escanea, recorta y envía img como pdf.

• `{i}pdsave <reply to image/pdf>`
    Escanea, recorta y guarda el archivo para fusionar u puede fusionar muchas páginas como un solo pdf.

• `{i}pdsend `
   Fusionar y enviar el PDF a recopilado de .pdsave.
"""

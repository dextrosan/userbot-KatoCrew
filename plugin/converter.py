...
• `{i}mtoi <reply to media>`
    convertir media a imágenes

• `{i}mtos <reply to media>`
    convertir medios en  sticker.

• `{i}doc <filename.ext>`
    Responder a un mensaje de texto para guardarlo en un archivo.
...
async def imak(event):
    ...
    if not inp:
        await eor(event, "Dar el nombre y la extensión del archivo")
        return
    ...
    if event.reply_to_msg_id:
        ...
        if a.media:
            ...
            try:
                ...
                await xx.edit(
                    f"**EL MENSAJE EXCEDE LOS LÍMITES DEL TELEGRAM**\n\nSo Pasted It On [NEKOBIN](https://nekobin.com/{key})"
                )
        else:
            return await eod(xx, "`Responder a un archivo legible`", time=5)
    else:
        return await eod(xx, "`Responder a un archivo legible`", time=5)
